...
try {
    ...
    def is_car_deploy_error = false;
      def projectpath=env.JOB_NAME
       def dynamicprojectname=projectpath.tokenize('/').reverse()[1]

 environment {
           envprojectname =${dynamicprojectname}
    ...
    stage("Checkout code: current project"){
        ...
        dir("${ei_service_project_tem_path}"){
            ...
            commithash = bat(returnStdout: true, script: 'git log')
 ...
 stage("Checkout code: configuration"){
    echo "Enter Checkout code: configuration"
      props = readYaml file: "${ei_service_project_tem_path}/SPLCICD/${dynamicprojectname}/jenkinsfile.pipeline.yml"

      echo " replay value is ${isReplay}"
    ...
    echo "environment variable name is ${env.envprojectname}"
 }
  echo "Base_EI_Project = ${Base_EI_Project}"
	//  if(!is_need_build){
...
//     }
    stage('Replace EI Serive Parameters') {
        dir ("${ei_service_project_tem_path}") {          
              dir ("SPLCICD/${dynamicprojectname}"){
                echo "Enter1"
                bat 'dir'                
                echo "enter2"
        ...
        stage("Build in Maven"){
          ...
          build_result = bat (returnStdout: true, script: "cd ${ei_service_project_tem_path}/SPLCICD/${dynamicprojectname} && mvn clean install -Dmaven.test.skip=true || echo success")
   ...
   { 
    stage('QA Team') {
       ...
       /* def body = emailTemplate([
            ...
            "status" :   "build is waiting for approval"
        ]);*/
                ...
                body: "The following details need your approval to move to another stage<br>user: ${user}<br><br>build number: ${currentBuild.number}<br><br>projectName: ${dynamicprojectname}<br><br>BranchName: ${env.BRANCH_NAME}<br><br>timeout: 1 minute"
               
                     
                                   
                //body: "user: "+ "${user}"+ \n +" is waiting for approval build number " +"${currentBuild.number}"+" for the project "+"${dynamicprojectname}"+" timeout will be in 1 minute"
        ...
        //  deploy_result =  
         powershell "copy ${ei_service_project_tem_path}/SPLCICD/${dynamicprojectname}/${dynamicprojectname}CompositeExporter/target/*.car C:/JenkinsShare"
        powershell "cd C:/temp; remove-item *.car"
        powershell "copy ${ei_service_project_tem_path}/SPLCICD/${dynamicprojectname}/${dynamicprojectname}CompositeExporter/target/*.car  C:/temp"
bat 'dir'
echo "*************neter***************"
...
powershell 'get-childitem -Path "C:/temp" | where-object { $_.Name -like "*.car" } | %{ rename-item -LiteralPath $_.FullName -NewName '+"${currentBuild.number}"+'_$($_.name)}'
 ...
 echo "Exit Cope car"
  ...
  //powershell "Copy-Item -Path C:/JenkinsShare/${dynamicprojectname}CompositeExporter_1.0.0.car -Destination C:/temp"
...
//build_result1 = powershell (returnStdout: true, script: "cd ei_service_project_tem_path/SPLCICD/SPLReleases; git pull main; git add . ; git commit -m 'commit'; git push origin HEAD:main ;echo success")
build_result1 = powershell (returnStdout: true, script: "cd ei_service_project_tem_path/SPLCICD/SPLReleases; git checkout '${env.BRANCH_NAME}';git status;git merge '${env.BRANCH_NAME}' -m 'Merge to main'; git add . ; git commit -m 'commit'; git push origin HEAD:main ;echo success")
...
bat 'dir'
        ...
        //     deploy_result = bat (returnStdout: true, script: "cd ${ei_service_project_tem_path}/HelloWorld && mvn  deploy -Dmaven.deploy.skip=true -Dmaven.car.deploy.skip=false -Dmaven.test.skip=true -Dmaven.wagon.http.ssl.insecure=true || echo success")
